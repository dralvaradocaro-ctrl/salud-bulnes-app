...
import { AlertTriangle, CheckCircle2, Droplets, ExternalLink, Plus, Trash2, XCircle } from 'lucide-react';
...
export default function RenalAntibioticReview({ open, onClose, records = [] }) {
  ...
  const patientOptions = useMemo(() => records.map(record => {
    ...
    return { record, form, key: record.id || record.bedCode, label: `${form.paciente || 'Sin nombre'} · ${form.servicio || ''} ${form.cama || record.bedCode || ''}`.trim() };
  }).sort((a, b) => a.label.localeCompare(b.label, 'es')), [records]);
  if (!open) return null;

  const selectPatient = key => {
    if (!key) { setPatient(emptyPatient); setAntibiotics(['']); return; }
  ...
  return <div className="fixed inset-0 z-[110] flex items-center justify-center bg-slate-950/65 p-2 backdrop-blur-sm sm:p-4" role="dialog" aria-modal="true" aria-label="Antibióticos y función renal">
    <div className="flex max-h-[94vh] w-full max-w-6xl flex-col overflow-hidden rounded-2xl border border-teal-200 bg-white shadow-2xl">
      ...
      <div className="min-h-0 flex-1 space-y-5 overflow-y-auto p-4 sm:p-6">
        <section className="rounded-xl border border-slate-200 bg-slate-50 p-4"><div className="grid gap-4 lg:grid-cols-[1.4fr_repeat(4,1fr)]"><div className="space-y-1.5"><Label>Paciente hospitalizado o modo libre</Label><select className="h-10 w-full rounded-md border border-slate-200 bg-white px-3 text-sm" value={patient.patientKey} onChange={event => selectPatient(event.target.value)}><option value="">Modo libre / datos manuales</option>{patientOptions.map(item => <option key={item.key} value={item.key}>{item.label}</option>)}</select></div><div className="space-y-1.5"><Label>Edad</Label><Input type="number" min="18" value={patient.edad} onChange={event => updatePatient('edad', event.target.value)} /></div><div className="space-y-1.5"><Label>Sexo</Label><select className="h-10 w-full rounded-md border border-slate-200 bg-white px-3 text-sm" value={patient.sexo} onChange={event => updatePatient('sexo', event.target.value)}><option value="">Seleccionar</option><option value="femenino">Femenino</option><option value="masculino">Masculino</option></select></div><div className="space-y-1.5"><Label>Peso actual (kg)</Label><Input type="number" min="1" step="0.1" value={patient.peso} onChange={event => updatePatient('peso', event.target.value)} placeholder="Para Cockcroft–Gault" /></div><div className="space-y-1.5"><Label>Creatinina (mg/dL)</Label><Input type="number" min="0.1" step="0.01" value={patient.creatinina} onChange={event => updatePatient('creatinina', event.target.value)} /></div></div>{patient.fechaCreatinina && <p className="mt-2 text-xs text-slate-500">Creatinina precargada del {patient.fechaCreatinina}.</p>}<div className="mt-3 flex flex-wrap gap-4"><label className="flex items-center gap-2 text-sm font-semibold text-slate-700"><input type="checkbox" checked={patient.aki} onChange={event => updatePatient('aki', event.target.checked)} />Función renal inestable / lesión renal aguda</label><label className="flex items-center gap-2 text-sm font-semibold text-slate-700"><input type="checkbox" checked={patient.dialysis} onChange={event => updatePatient('dialysis', event.target.checked)} />Hemodiálisis, diálisis peritoneal o TRRC</label></div></section>
