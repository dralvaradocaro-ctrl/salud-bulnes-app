...
function initialForm(patient, bed) {
  return {
    ...
    capacidad: 'directa', representanteNombre: '', representanteRut: '', representanteRelacion: '', representanteTelefono: '', conversacion: '',
...
function documentMarkup(form) {
  return `<article class="clinical-document">
    ...
    <section><h2>6. Participación e información entregada</h2><p>${checked(form.capacidad === 'directa')} Paciente participa directamente &nbsp; ${checked(form.capacidad === 'parcial')} Paciente participa junto a familia &nbsp; ${checked(form.capacidad === 'representante')} Participa representante/familia</p><p><b>Representante:</b> ${escapeHtml(form.representanteNombre || '—')} · <b>RUT:</b> ${escapeHtml(form.representanteRut || '—')} · <b>Relación:</b> ${escapeHtml(form.representanteRelacion || '—')} · <b>Teléfono:</b> ${escapeHtml(form.representanteTelefono || '—')}</p><p>Se explicó la condición clínica, el pronóstico, las alternativas disponibles, los beneficios y cargas de las intervenciones, la continuidad de los cuidados y la posibilidad de reevaluar la decisión ante cambios clínicos relevantes.</p><p><b>Registro de la conversación:</b><br>${lines(form.conversacion)}</p></section>
    <section class="signatures"><h2>7. Firmas y registro</h2><div class="signature-grid"><div><i></i><b>Paciente</b><span>${escapeHtml(form.pacienteNombre || form.nombre || '')}</span><small>Firma</small></div><div><i></i><b>Representante / familiar</b><span>${escapeHtml(form.representanteNombre || '')}</span><small>Firma y relación</small></div><div><i></i><b>Médico/a tratante</b><span>${escapeHtml(form.medicoNombre || '')}</span><small>Firma y RUT: ${escapeHtml(form.medicoRut || '')}</small></div><div><i></i><b>Segundo profesional</b><span>${escapeHtml(form.segundoNombre || '')}</span><small>${escapeHtml(form.segundoProfesion || 'Firma y profesión')}</small></div></div><p><b>Observaciones:</b> ${lines(form.observaciones)}</p><p class="reevaluation">Decisión clínica individualizada. Reevaluar ante cambios relevantes en la condición, el pronóstico o los objetivos de cuidado.</p></section>
  </article>`;
}

const documentCss = `
  *{box-sizing:border-box}.clinical-document{color:#111827;background:#fff;font-family:Arial,Helvetica,sans-serif;font-size:10.5pt;line-height:1.36}.doc-header{display:grid;grid-template-columns:86px 1fr 150px;align-items:center;gap:14px;border-bottom:2px solid #1e3a5f;padding-bottom:9px;margin-bottom:18px}.doc-header img{max-width:80px;max-height:58px;object-fit:contain}.doc-header strong{display:block;color:#17365d;font-size:11pt}.doc-header span{font-size:9pt}.doc-header aside{text-align:right;font-size:8.5pt}.clinical-document h1{text-align:center;font-size:15pt;color:#17365d;margin:0 0 2px}.subtitle{text-align:center;font-size:10pt;margin:0 0 17px;color:#475569}.clinical-document section{break-inside:avoid;margin:0 0 13px}.clinical-document h2{font-size:10.5pt;color:#17365d;background:#edf3f8;border-left:4px solid #376a94;padding:4px 7px;margin:0 0 7px}.doc-grid{display:grid;grid-template-columns:1.7fr 1fr;gap:2px 18px}.clinical-document p{margin:5px 0}.plan-box{border:1px solid #94a3b8;padding:6px 10px}.plan-box p{padding:3px 5px}.plan-box .selected{background:#edf3f8;color:#17365d}.note{font-size:8.5pt;color:#475569}.clinical-document table{width:100%;border-collapse:collapse;font-size:9pt}.clinical-document th,.clinical-document td{border:1px solid #94a3b8;padding:5px;text-align:left;vertical-align:top}.clinical-document th{width:31%;background:#f8fafc}.signature-grid{display:grid;grid-template-columns:1fr 1fr;gap:22px 32px;margin-top:18px}.signature-grid div{text-align:center;min-height:70px}.signature-grid i{display:block;border-top:1px solid #334155;margin-top:32px}.signature-grid b,.signature-grid span,.signature-grid small{display:block}.signature-grid small{font-size:8pt;color:#475569}.reevaluation{border-top:1px solid #94a3b8;padding-top:7px;font-size:8.5pt;font-weight:bold}
`;
    ...
    <div className="flex h-[97vh] w-full max-w-[1500px] flex-col overflow-hidden rounded-2xl bg-white shadow-2xl">
      ...
      <div className="grid min-h-0 flex-1 lg:grid-cols-[minmax(440px,0.82fr)_minmax(620px,1.18fr)]">
        <div className={`${preview ? 'hidden lg:block' : 'block'} min-h-0 overflow-y-auto p-5`}>
          ...
          <section className="mt-5"><h3 className="mb-3 text-sm font-black text-indigo-950">Conversación y profesionales</h3><div className="grid gap-3 sm:grid-cols-2"><Field label="Participación" wide><select className={input} value={form.capacidad} onChange={e => update('capacidad', e.target.value)}><option value="directa">Paciente participa directamente</option><option value="parcial">Paciente participa junto a familia</option><option value="representante">Participa representante / familia</option></select></Field><Field label="Representante / familiar"><input className={input} value={form.representanteNombre} onChange={e => update('representanteNombre', e.target.value)} /></Field><Field label="Relación"><input className={input} value={form.representanteRelacion} onChange={e => update('representanteRelacion', e.target.value)} /></Field><Field label="Médico/a tratante"><input className={input} value={form.medicoNombre} onChange={e => update('medicoNombre', e.target.value)} /></Field><Field label="RUT profesional"><input className={input} value={form.medicoRut} onChange={e => update('medicoRut', e.target.value)} /></Field><Field label="Registro de la conversación" wide><textarea className={textarea} value={form.conversacion} onChange={e => update('conversacion', e.target.value)} /></Field><Field label="Segundo profesional"><input className={input} value={form.segundoNombre} onChange={e => update('segundoNombre', e.target.value)} /></Field><Field label="Profesión"><input className={input} value={form.segundoProfesion} onChange={e => update('segundoProfesion', e.target.value)} /></Field></div></section>
        </div>
      ...
      <footer className="flex flex-wrap items-center justify-between gap-3 border-t border-slate-200 bg-white px-5 py-3"><p className="text-[11px] text-slate-500">Revise la previsualización antes de firmar. Ajuste el contenido al caso y a la normativa institucional vigente.</p><div className="flex gap-2"><Button variant="outline" onClick={onClose}>Cerrar</Button><Button onClick={() => printDocument(form)} className="gap-2 bg-indigo-700 hover:bg-indigo-800"><Printer className="h-4 w-4" />Imprimir / guardar PDF</Button></div></footer>
    </div>
  </div>;
}
