    ...
    <div className="flex h-[97vh] w-full max-w-[1500px] flex-col overflow-hidden rounded-2xl bg-white shadow-2xl">
      ...
      <div className="grid min-h-0 flex-1 lg:grid-cols-[minmax(430px,.82fr)_minmax(620px,1.18fr)]">
        <div className={`${preview ? 'hidden lg:block' : 'block'} min-h-0 overflow-y-auto p-5`}>
          {reports.length > 0 && <section className="mb-5 rounded-xl border border-slate-200 bg-slate-50 p-3"><h3 className="mb-2 text-sm font-black text-slate-900">Informes almacenados ({reports.length})</h3><div className="max-h-44 space-y-2 overflow-y-auto">{reports.map(report => <button key={report.id || report.createdAt} type="button" onClick={() => { setForm({ ...initialForm(patient, bed), ...report }); setSaveMessage('Versión almacenada cargada para consultar o reimprimir.'); }} className="flex w-full items-center justify-between gap-3 rounded-lg border border-slate-200 bg-white p-2 text-left hover:border-emerald-300"><span className="min-w-0"><b className="block truncate text-xs text-slate-800">{report.title || REPORT_TYPES[report.type]?.label || 'Informe médico'}</b><span className="text-[11px] text-slate-500">{new Date(report.createdAt || report.fecha).toLocaleString('es-CL')} · {report.medico || 'Profesional no consignado'}</span></span><span className="inline-flex shrink-0 items-center gap-1 text-[11px] font-bold text-emerald-800"><Printer className="h-3.5 w-3.5" />Reimprimir</span></button>)}</div></section>}
