...
function RuledText({ value, lines }) {
  ...
  const estimatedWrappedLines = Math.max(explicitLines, Math.ceil(text.length / 82));
  const renderedLines = Math.max(lines, estimatedWrappedLines + 1);
  return <div className="hodom-ruled-text" style={{ minHeight: `${renderedLines * 6.35}mm` }}>
    <div className="hodom-rule-lines" aria-hidden="true">{Array.from({ length: renderedLines }, (_, index) => <span key={index} style={{ top: `${(index + 1) * 6 + 1.2}mm` }} />)}</div>
...
export default function FormulariosHODOM() {
  ...
  useEffect(() => { const p = getMultiPrefill(); if (p) { setPrefillContext(p); setF(old => ({ ...old, nombre: p.patient_name || '', rut: formatRut(p.patient_rut), edad: p.edad || '', telefono: p.patient_telefono || '', domicilio: p.patient_direccion || '', antecedentes: p.antecedentes_relevantes || p.antecedentes || '', anamnesis: p.resumen_caso || p.clinical_text || '', diagnosticos: p.diagnostico_principal || p.diagnostico || '', diagnosticoConsentimiento: p.diagnostico_principal || p.diagnostico || '' })); } }, []);
  ...
  return <>
    <style>{`
      ...
      @media print{html,body,#root{background:#fff!important;color:#000!important}.hodom-screen{display:none!important}.hodom-print{display:block!important}.hodom-doc,.hodom-sheet,.hodom-ruled-text{background:#fff!important}.hodom-rule-lines span{background:#555!important}.hodom-sheet{height:auto!important;min-height:279mm;margin:0;box-shadow:none;break-after:page;overflow:visible}.hodom-sheet:last-child{break-after:auto}}
